...
def question1(shape):
    """
    Define a function to return a tensor where all elements are the number one with a given shape
    """
...
def question8(y, w, b):
    """
    Define a function that calculates batch (w^T)*(y - b)
...
def question9(x):
    """
    Given a 3-D tensor x (b, n, m), calculate the average of the tensor elements along the
    last dimension without accounting for the paddings (0 values).
    DO NOT use loop, list comprehension, or any other similar operations.
    """
...
def question10(pairs):
    """
    Implement a funtion that calculates the Euclidean distance of each vector pair.
    """
